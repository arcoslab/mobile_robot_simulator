...
class Differential_robot(object):
    def __init__(self):
        ...
        self.robot_planes=[[( 1., 0., 0),(self.robot_dim[0]/2.,0.,0.) ], #(normal, point)
        ...
        self.d=self.robot_dim[1]/2.0 #distance between robot center and descentralized point
        
        #inertial constants
        self.weight_force=2.
        self.object_mass=self.weight_force/9.8
        self.object_rotational_inertia=self.object_mass*(self.robot_dim[0]**2+self.robot_dim[1]**2)/12.
        self.max_wheel_vel=100

    def set_params(self, robot_dim, robot_center, wheel_radius, wheel_distance, center_distance, weight, max_vel):
    ...
    def wheel_velocity(self, descentralized_velocity, angle):
        #compute wheel velocities from the required velocity of the descentralized point, and the current angle of the robot.
        right_wheel =(( self.d*cos(angle)-0.5*self.b*sin(angle) )*descentralized_velocity[0] +( self.d*sin(angle)+0.5*self.b*cos(angle) )*descentralized_velocity[1])/(self.d*self.r)
        left_wheel=(( self.d*cos(angle)+0.5*self.b*sin(angle) )*descentralized_velocity[0] +( self.d*sin(angle)-0.5*self.b*cos(angle) )*descentralized_velocity[1])/(self.d*self.r)
    ...
    def central_velocity(self, descentralized_point_vel, angle):
        #compute the velocity at the center of the robot, by knowing the required velocity and current angle
        [phi_l, phi_r]=self.wheel_velocity(descentralized_point_vel, angle)
        central_velocity=array([0.,0.,0.,0.,0.,0.])
        central_velocity[0]=((self.r*cos(angle))/self.b)*(phi_r+phi_l)
        central_velocity[1]=((self.r*sin(angle))/self.b)*(phi_r+phi_l)
