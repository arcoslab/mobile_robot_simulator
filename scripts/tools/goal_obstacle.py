#!/usr/bin/env python
# Copyright (c) 2016 Autonomous Robots and Cognitive Systems Laboratory.
...
# arcospyu
from arcospyu.yarp_tools.yarp_comm_helpers import new_port
from arcospyu.yarp_tools.yarp_comm_helpers import write_narray_port
from arcospyu.rawkey.rawkey import Raw_key, Keys
from arcospyu.control.control_loop import Controlloop
from arcospyu.kdl_helpers import my_adddelta
...
class Control(Controlloop):
    ...
    def process(self):
        ...
        if self.random_movement:
            for i in range(self.obstacle_cant):
                self.obstacle_vel_list[i][:2] -= 0.1 * self.obstacle_vel_list[
                    i][:2]
        ...
        for i in range(self.robot_cant):
            self.goal_pose_list[i] = my_adddelta(self.goal_pose_list[i],
                                                 self.goal_vel_list[i], dt)
            write_narray_port(self.goal_out_port_list[i],
                              self.goal_pose_list[i][:3, 3])
        for i in range(self.obstacle_cant):
            self.obstacle_pose_list[i] = my_adddelta(
                self.obstacle_pose_list[i], self.obstacle_vel_list[i], dt)
            write_narray_port(self.obstacle_out_port_list[i],
                              self.obstacle_pose_list[i][:3, 3])
