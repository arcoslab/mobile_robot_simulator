...
#this script takes the position of the goal, and the position of the obstacles and computes the direction a robot in a particular point needs.
...
def vectorfield(goal_pos, obstacle_pos_list, robot_pos, robot_vel, dt, goal_constant=1., obstacle_constant=1., damp_constant=0.5):
    ...
    for i in range(obstacle_cant):
        ...
        obstacle_force+=obstacle_constant*(robot_pos-obstacle_pos_list[i])/(obstacle_distance_list[i]**3)

    if goal_distance<1:
        damp_constant=goal_distance*(damp_constant-1)/(1-0.3)+(1-0.3*damp_constant)
